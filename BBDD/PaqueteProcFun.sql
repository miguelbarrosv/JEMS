...
PROCEDURE PROC_VERMIEMBROSEQUIPO(P_EQUIPO_ID IN EQUIPO.COD_EQUIPO%TYPE, MIEMBROS OUT NUMBER);
PROCEDURE PROC_REF_EQUIPO(CUR_EQUIPO OUT SYS_REFCURSOR);
PROCEDURE GENERAR_CALENDARIO(FECHA_INICIO_LIGA DATE);
END PAQ_PROC_FUN;
...
FUNCTION FUN_CALCULARJUGADORES--ESTA FUNCION VA A CALCULAR EL NUMERO DE JUGADORES POR EQUIPO
...
PROCEDURE PROC_VERMIEMBROSEQUIPO --PROCEDIMIENTO CON EL CUAL MOSTRAMOS LOS JUGADORES DE CADA EQUIPO UTILIZANDO
...
IS
 BEGIN      
  OPEN CUR_EQUIPO FOR
    SELECT E.COD_EQUIPO,E.NOMBRE,E.NACIONALIDAD,E.PRESUPUESTO,E.PUNTUACION,E.DUEÑO_COD_DUEÑO"CODIGO_DUEÑO",D.NOMBRE"NOMBRE_DUEÑO",NVL(COUNT(J.COD_JUGADOR),0)"NUM_JUGADORES"
    FROM EQUIPO E,DUEÑO D,JUGADOR J
    WHERE E.COD_EQUIPO=J.EQUIPO_COD_EQUIPO(+)
    AND E.DUEÑO_COD_DUEÑO(+)=D.COD_DUEÑO 
    GROUP BY E.COD_EQUIPO, E.NOMBRE, E.NACIONALIDAD, E.PRESUPUESTO, E.PUNTUACION,E.DUEÑO_COD_DUEÑO, D.NOMBRE;
END PROC_REF_EQUIPO;

PROCEDURE GENERAR_CALENDARIO(FECHA_INICIO_LIGA DATE)--CREACION DE CALENDARIO VACIO
IS 
...
BEGIN 
    ...
    ELSE
        INSERT INTO LIGA (FECHA_INICIO,NOMBRE,ESTADO) 
        VALUES(FECHA_INICIO_LIGA,'HOLA',0); --INSERTAMOS LA FECHA DE INICIO DE LA LIGA
